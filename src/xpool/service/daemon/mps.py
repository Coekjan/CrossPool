...
def probe_mps_controller() -> MpsProbeResult:
    ...
    if raw_pipe_directory is None or not raw_pipe_directory.strip():
        return MpsProbeResult(False, "CUDA_MPS_PIPE_DIRECTORY is required for MPS controller readiness")
    ...
    try:
        ...
        return MpsProbeResult(False, f"failed to execute serialized MPS controller probe: {exc}")
