...
template <typename T> XPOOL_DEVICE_FN inline cuda::atomic_ref<T, cuda::thread_scope_system> system_atomic(T &value) {
...
template <typename T> XPOOL_DEVICE_FN inline T load_acquire(T &value) {
...
template <typename T, typename Desired> XPOOL_DEVICE_FN inline void store_release(T &value, Desired desired) {
  static_assert(std::is_convertible_v<Desired, T>);
  system_atomic(value).store(static_cast<T>(desired), cuda::memory_order_release);
}
...
XPOOL_DEVICE_FN inline T compare_exchange_acq_rel(T &value, Expected expected, Desired desired) {
  static_assert(std::is_convertible_v<Expected, T>);
  static_assert(std::is_convertible_v<Desired, T>);
  auto observed = static_cast<T>(expected);
  (void)system_atomic(value).compare_exchange_strong(observed, static_cast<T>(desired), cuda::memory_order_acq_rel,
