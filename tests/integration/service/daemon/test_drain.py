...
from tests.harness.service.daemon import (
    ...
    devagent_transport_arenas_drain_path,
...
from xpool.service.wire import DevagentTransportArenaDrainResponse, ProcessRef, TransportArenaHandleRecord
...
def test_daemon_replacement_generation_terminates_all_live_lease_owners_concurrently(
    ...
    devagent_process, devagent_id = start_sleeping_proc()
    ...
    devagent = devagent_registration(cuda_device=0, pid=devagent_id.pid)
    try:
        ...
        bindings = devagent_transport_arena_bindings(("a", 0), ("b", 0))
        ...
        assert (
            request(
                ...
                json={"publisher": process_ref(devagent), "bindings": bindings},
            ).status_code
        ...
        stop_proc(devagent_process)
        replacement = devagent_registration(cuda_device=0)
        response = request(app, "POST", "/devagent/register", json=replacement)

        assert response.status_code == HTTPStatus.NO_CONTENT
        assert request(app, "GET", "/instances").json() == []
    finally:
        if devagent_process.poll() is None:
            stop_proc(devagent_process)
...
def test_daemon_drain_devagent_transport_arenas_blocks_new_acquires_and_terminates_stale_owner(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    drain = request(
        app,
        "POST",
        devagent_transport_arenas_drain_path(0),
        json=process_ref(devagent),
    )
    republish = request(
        app,
        "POST",
    ...
    assert republish.json()["detail"] == {
        "kind": "not_ready",
        "message": "devagent transport arenas are terminating",
    }
    ...
    assert heartbeat.json()["warnings"] == [
        {
            ...
            "cuda_device": 0,
            "message": "devagent on CUDA device 0 is terminating",
        }
    ]
    assert response.status_code == HTTPStatus.SERVICE_UNAVAILABLE
    assert response.json()["detail"] == {
        "kind": "not_ready",
        "message": "local attention devagent transport arenas are terminating",
    }
...
def test_daemon_drain_waits_for_inflight_transport_arena_acquire(
    ...
    def drain() -> None:
        drain_entered.set()
        try:
            drain_result["response"] = state.drain_devagent_transport_arenas(
                0,
                ProcessRef.model_validate(process_ref(devagent)),
            )
        except BaseException as exc:
    ...
    assert isinstance(response, DevagentTransportArenaDrainResponse)
...
def test_daemon_allows_stale_devagent_to_drain_transport_arenas(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = XpoolConfig.from_file("configs/xpool.example.toml")
    app = create_app(config)
