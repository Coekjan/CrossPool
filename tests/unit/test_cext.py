...
def reset_cext_loader(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(cext_module, "native_ops_loaded", False)
    monkeypatch.setattr(cext_module, "import_module", lambda name: None)
...
def test_native_loader_is_serialized(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_load_library(path: str) -> None:
        with events_lock:
            events.append("load")

    monkeypatch.setattr(cext_module, "files", lambda package: FakeFiles(package))
    monkeypatch.setattr(cext_module, "as_file", lambda resource: FakeAsFileContext(library_path))
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        tuple(executor.map(lambda index: ensure_xpool_ops_loaded(), range(32)))
...
def test_native_loader_checks_native_abi_version(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cext_module, "files", lambda package: FakeFiles(package))
    monkeypatch.setattr(cext_module, "as_file", lambda resource: FakeAsFileContext(library_path))
    monkeypatch.setattr(torch.ops, "load_library", lambda path: events.append("load"))
    monkeypatch.setattr(torch.ops, "xpool", FakeOpNamespace(events, abi_version=ABI_VERSION), raising=False)
...
def test_native_loader_rejects_python_graph_wrapper_import_failure(
    ...
    monkeypatch.setattr(cext_module, "files", lambda package: FakeFiles(package))
    monkeypatch.setattr(cext_module, "as_file", lambda resource: FakeAsFileContext(library_path))
    monkeypatch.setattr(torch.ops, "load_library", lambda path: None)
    monkeypatch.setattr(torch.ops, "xpool", SimpleNamespace(abi_version=lambda: ABI_VERSION), raising=False)
    ...
    assert cext_module.native_ops_loaded is False
...
def test_native_loader_loads_package_resource_while_as_file_context_is_active(
    ...
    library_path = tmp_path / "libxpool_cext.so"
    library_path.write_bytes(b"")
    context = FakeAsFileContext(library_path)
    events: list[bool] = []
    ...
    monkeypatch.setattr(cext_module, "files", lambda package: FakeFiles(package))
    monkeypatch.setattr(cext_module, "as_file", lambda resource: context)
    monkeypatch.setattr(torch.ops, "load_library", fake_load_library)
    monkeypatch.setattr(torch.ops, "xpool", SimpleNamespace(abi_version=lambda: ABI_VERSION), raising=False)
...
def test_native_loader_rejects_missing_abi_version_op(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(cext_module, "files", lambda package: FakeFiles(package))
    monkeypatch.setattr(cext_module, "as_file", lambda resource: FakeAsFileContext(Path(__file__)))
    monkeypatch.setattr(torch.ops, "load_library", lambda path: None)
...
class FakeAsFileContext(AbstractContextManager[Path]):
