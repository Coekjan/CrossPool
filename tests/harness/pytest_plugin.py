...
import xpool.config
from tests.harness.bootstrap import TestBootstrapError, ensure_test_native
from tests.harness.requirements import (
    CudaRequirement,
    RequirementGuard,
    RequirementResolver,
)
from tests.harness.test_plan import (
    CollectedTestCase,
    TestPlan,
    TestRequirements,
    TestStage,
...
def token_parity_group(item: pytest.Item) -> TokenParityGroupRef | None:
    ...
    return TokenParityGroupRef(name=name, expected_case_count=expected_case_count)
    ...
    for item in session.items:
        ...
        cases.append(
            CollectedTestCase(
                ...
                token_parity_group=token_parity_group(item),
            )
        )
    try:
