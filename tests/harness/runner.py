...
from tests.harness.execution_task import ExecutionTask, compile_execution_tasks
from tests.harness.gpu import GpuLease, GpuPool
from tests.harness.pytest_report import PytestCaseStatus, PytestTaskReport
from tests.harness.sglang.parity import (
    TOKEN_PARITY_ARTIFACT_FILENAME,
    TokenParityArtifact,
    TokenParityGroupResult,
    TokenParityGroupStatus,
    evaluate_token_parity_group,
)
...
from tests.harness.test_plan import CollectedTestCase, TestPlan, TestStage
...
class TaskOutcome:
    ...
    def result_code(self) -> int:
        ...
        if self.completion.kind in {TaskCompletionKind.TIMED_OUT, TaskCompletionKind.LEAKED}:
            return 1
        if self.completion.kind is TaskCompletionKind.INFRASTRUCTURE_FAILED:
            return 2
...
class SuiteRunner:
    ...
    def run(self) -> int:
        ...
        try:
            ...
            parity_results = self.token_parity_results()
            self.report_token_parity(parity_results)
            parity_code = max((result.result_code for result in parity_results), default=0)
            return self.result_code(max(e2e_code, parity_code))
    ...
    def token_parity_results(self) -> tuple[TokenParityGroupResult, ...]:
        """Classify every group with complete ordinary pytest case outcomes."""

        groups: dict[str, list[CollectedTestCase]] = defaultdict(list)
        task_by_nodeid = {case.nodeid: task for task in self.tasks for case in task.cases}
        for case in self.plan.cases:
            if case.token_parity_group is not None:
                groups[case.token_parity_group.name].append(case)
        results: list[TokenParityGroupResult] = []
        for group, cases in sorted(groups.items()):
            ...
            if all(report.status is PytestCaseStatus.PASSED for report in reports):
                try:
                    artifacts = tuple(
                        TokenParityArtifact.read(outcome.directory / "artifacts" / TOKEN_PARITY_ARTIFACT_FILENAME)
                        for outcome in outcomes
                        if outcome is not None
                    )
                except (OSError, ValueError) as error:
                    results.append(TokenParityGroupResult(group, TokenParityGroupStatus.INCONSISTENT, str(error)))
                    continue
            results.append(evaluate_token_parity_group(group, reports, artifacts))
        return tuple(results)

    @staticmethod
    def report_token_parity(results: tuple[TokenParityGroupResult, ...]) -> None:
        """Print one concise diagnostic for every classified parity group."""

        for result in results:
            message = f"{result.status.value.upper()} token parity group {result.group}"
