...
def test_plan_round_trips_strict_unversioned_json(tmp_path: Path) -> None:
    plan = tests.harness.test_plan.TestPlan((e2e_case("eager"), e2e_case("full")))
    path = tmp_path / "plan.json"

    plan.write(path)

    assert tests.harness.test_plan.TestPlan.read(path) == plan
...
def test_plan_rejects_unknown_case_fields(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="exactly"):
        tests.harness.test_plan.TestPlan.read(path)
...
def test_collected_case_rejects_stage_and_resource_drift() -> None:
    ...
    with pytest.raises(ValueError, match="unit tests cannot require CUDA"):
        tests.harness.test_plan.CollectedTestCase(
            ...
            requirements=tests.harness.test_plan.TestRequirements(1, False, False, ()),
            estimated_duration_seconds=None,
            timeout_seconds=10,
            token_parity_group=None,
        )
...
def test_plan_rejects_inconsistent_parity_group_cardinality() -> None:
    ...
    full = tests.harness.test_plan.CollectedTestCase(
        ...
        token_parity_group=tests.harness.test_plan.TokenParityGroupRef("example", 3),
    )

    with pytest.raises(ValueError, match="inconsistent expected_case_count"):
        tests.harness.test_plan.TestPlan((eager, full))
...
def e2e_case(mode: str) -> tests.harness.test_plan.CollectedTestCase:
    path = "tests/suites/e2e/test_e2e_example.py"
    return tests.harness.test_plan.CollectedTestCase(
        path=path,
        nodeid=f"{path}::test_e2e_example[{mode}]",
        stage=tests.harness.test_plan.TestStage.E2E,
        requirements=tests.harness.test_plan.TestRequirements(2, True, True, ("organization/model",)),
        estimated_duration_seconds=60,
        timeout_seconds=120,
        token_parity_group=tests.harness.test_plan.TokenParityGroupRef("example", 2),
    )
