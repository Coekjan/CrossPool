...
struct TestArena {
  xpool::utils::trace::BufferState state;
...
XPOOL_KERNEL_FN void reserve_trace_records(TestArena *arena, std::uint64_t *reservations) {
  const auto buffer =
      xpool::utils::trace::BufferView<TestRecord>{cuda::std::span{arena->records, std::size_t{2}}, arena->state};
