...
import tests.harness.sglang.plugin
...
def test_plugin_applies_adapter_owned_hooks_with_pinned_sglang_registry(
    ...
    install_fake_hook(monkeypatch, "tests.harness.sglang.plugin.minimal_config")

    xpool.integrations.sglang.plugin.install()
    HookRegistry.apply_hooks()

    assert tests.harness.sglang.plugin.minimal_config() == "patched"


def test_plugin_apply_hooks_guard_fails_closed_with_pinned_sglang_registry(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    install_fake_hook(monkeypatch, "tests.harness.sglang.plugin.missing_target")

    xpool.integrations.sglang.plugin.install()
    with pytest.raises(SystemExit, match=r"tests\.harness\.sglang\.plugin\.missing_target"):
        HookRegistry.apply_hooks()
...
def install_fake_hook(monkeypatch: pytest.MonkeyPatch, target: str) -> None:
    ...
    for constant, name in target_names.items():
        monkeypatch.setattr(
            ...
            constant,
            f"tests.harness.sglang.plugin.{name}",
        )
        monkeypatch.setattr(
            tests.harness.sglang.plugin,
            name,
            getattr(tests.harness.sglang.plugin, name),
        )
    monkeypatch.setattr(
        tests.harness.sglang.plugin,
        "minimal_config",
        tests.harness.sglang.plugin.minimal_config,
    )
    monkeypatch.setattr(
        xpool.integrations.sglang.plugin, "init_global_config", tests.harness.sglang.plugin.minimal_config
    )
    monkeypatch.setattr(
        xpool.integrations.sglang.plugin,
